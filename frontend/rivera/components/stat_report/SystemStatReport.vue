<template>
	<div>
		...
		<div class="form-row align-items-center">
			<div class="form-group col-md-5 m-1"></div>
			<div class="form-group col-md-2 m-1 mt-3" style="text-align: center">
				<b-button pill variant="success" @click="loadAttendance" class="w-25"
					>Load</b-button
				>
			</div>
		</div>
...
export default {
	...
	methods: {
		loadAttendance() {
			...
			this.$axios
				.get(
					`/api/admin/system-income?&startDate=${this.startDate+"T00:00:00"}&endDate=${this.endDate+"T00:00:00"}&type=${this.type}`
				)
				.then((response) => {
